...
inline int MyDebug(const char* format, ...)
{
	va_list aptr;
	va_start(aptr, format);
	int bufferSize = 260;
	char* buffer = new char[bufferSize];    // 缓冲区
	int r = vsprintf_s(buffer, bufferSize, format, aptr);
	while (r == -1)
	{
		bufferSize *= 2;    // 走到这就是缓冲区不够大, 每次递增2倍缓冲区
		char* p = new char[bufferSize];
		delete[] buffer;
		buffer = p;
		r = vsprintf_s(buffer, bufferSize, format, aptr);   // 继续格式化
	}
	OutputDebugStringA(buffer); // 输出
	delete[] buffer;
	return r;
}
